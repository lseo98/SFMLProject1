...
private:
    ...
    void updateTexture() override {
        ...
        if (stageNumber == 3 || stageNumber == 4) {
            if (direction == 1) {
                textureRect = sf::IntRect(200, 0, 800, 435); // 오른쪽 이미지 영역
                image("land_elite_unit_right.png", textureRect);
            }
            else if (direction == -1) {
                textureRect = sf::IntRect(0, 0, 800, 435); // 왼쪽 이미지 영역
                image("land_elite_unit_left.png", textureRect);
            }
        }
    }
};
